...
async def generate_text(prompt: str, model: str = "llama3", language: str = "russian") -> dict:
...
async def predict_diseases(
    ...
    correlations_text = ""
    if symptom_correlations:
        correlations_text = "Известные корреляции симптомов и заболеваний из базы данных:\n"
        for corr in symptom_correlations:
            correlations_text += f"- Симптом '{corr.symptom}' → Заболевание '{corr.disease}' (сила связи: {corr.correlation_strength or 'неизвестно'})"
            if corr.notes:
                correlations_text += f" — {corr.notes}"
            correlations_text += "\n"
