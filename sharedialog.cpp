...
ShareDialog::ShareDialog(int personId, const QString &personName,
    ...
    : QDialog(parent), m_personId(personId), m_personName(personName), m_api(api)
{
...
void ShareDialog::onShareClicked() {
    ...
    m_api->post(QString("/api/db/persons/%1/share").arg(m_personId), body, [this](const ApiResult &r) {
        if (!r.ok) {
